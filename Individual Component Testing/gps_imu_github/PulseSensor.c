...
void get_pulse2() {
	++samples_since_last_beat;
	int N = samples_since_last_beat * SAMPLE_PERIOD;
	if (val2 < thresh2 && N > (IBI / 5) * 3) { // avoid dicrotic noise by waiting 3/5 of last IBI
		if (val2 < trough_amp) {                        // T is the trough
		  trough_amp2 = val2;                            // keep track of lowest point in pulse wave
		}
	  }

	  if (val2 > thresh2 && val2 > peak_amp) {       // thresh condition helps avoid noise
		  peak_amp2 = val2;                              // P is the peak
	  }                                          // keep track of highest point in pulse wave

	  //  NOW IT'S TIME TO LOOK FOR THE HEART BEAT
	  // signal surges up in value every time there is a pulse
	  if (N > 250) {                             // avoid high frequency noise
		if ( (val2 > thresh2) && (pulse == false) && (N > ((IBI / 5) * 3)) ) {
		  pulse = true;                             // set the Pulse flag when we think there is a pulse
		  IBI = N;    // measure time between beats in mS
		  samples_since_last_beat = 0;
	  ...
	  if (val2 < thresh2 && pulse) {  // when the values are going down, the beat is over
		pulse = false;                         // reset the Pulse flag so we can do it again
		amp2 = peak_amp2 - trough_amp2;                           // get amplitude of the pulse wave
		thresh2 = amp2 / 2 + trough_amp2;                  // set thresh at 50% of the amplitude
		peak_amp2 = thresh2;                            // reset these for next time
		trough_amp2 = thresh2;
	  }

	  if (N > 2500) {                          // if 2.5 seconds go by without a beat
		thresh2 = 2400;                // set thresh default
		peak_amp2 = 2400;                               // set P default
		trough_amp2 = 2400;                               // set T default
		samples_since_last_beat = 0;          // bring the lastBeatTime up to date
		first_beat = true;                      // set these to avoid noise
		second_beat = false;                    // when we get the heartbeat back
		BPM = 0;
