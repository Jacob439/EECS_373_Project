...
void lin_acc_vec(I2C_HandleTypeDef *hi2c, float* vec){
	vec[0] = x_lin_acc_raw(hi2c) * 4.0f / 32678.0f;
	vec[1] = y_lin_acc_raw(hi2c) * 4.0f / 32678.0f;
	vec[2] = z_lin_acc_raw(hi2c) * 4.0f / 32678.0f;
	return;
}

float x_lin_acc(I2C_HandleTypeDef *hi2c){
	return x_lin_acc_raw(hi2c) * 4.0f / 32678.0f;
}

float y_lin_acc(I2C_HandleTypeDef *hi2c){
	return y_lin_acc_raw(hi2c) * 4.0f / 32678.0f;
}

float z_lin_acc(I2C_HandleTypeDef *hi2c){
	return z_lin_acc_raw(hi2c) * 4.0f / 32678.0f;
}
