...
//	}
	...
	//printf("%c", rx_data[0]);
	if (rx_data[0] == '\n' && GO == 1 /*&& rx_index == 69*/){ // if we have the id string send it to temp for compare
		GO = 0;
		rx_index = 0;
		for (int i = 0; i < 6; ++i){
		  temp[i] = rx_buffer[i];
	...
	HAL_StatusTypeDef ret = HAL_UART_Receive_IT(&huart1, rx_data, 1);
	if (ret == HAL_TIMEOUT) {
		printf("wtf is going on");
	}
}
double get_distance(){
//	printf("p2p:%f", p2p_dist);
