// example use of pulsesensor library
// by Mason
...
// flag used for polling the pulse sensor
static bool update_pulse_sensor_flag = false;
/* USER CODE END PV */
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef* htim){
  ...
  // if calling in timer callback
	updatePulseSensor();

  // if using polling
  update_pulse_sensor_flag = true;
}
...
int main(void)
{
  ...
  while (1)
  {
    // polling pulse sensor
    if (update_pulse_sensor_flag) {
      updatePulseSensor();
    	update_pulse_sensor_flag = false;
    }
  }
  /* USER CODE END 3 */
}
